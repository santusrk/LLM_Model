...
def run_validation(model,validation_ds,tokenizer_src,tokenizer_tgt,max_len,device,print_msg,global_state, writer,num_exapmles=2):
    model.eval()
    count=0

    console_width = 80 # size of the control window -- just use a defalut value
    with torch.no_grad():
        for batch in validation_ds:
            count+=1
            encoder_input = batch['encoder_input'].to(device)
            encoder_mask = batch['enocoder_mask'].to(device)
